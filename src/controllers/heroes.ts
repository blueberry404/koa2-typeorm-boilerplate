...
import { IHeroUpdateRequest, IHeroIDRequest } from '../interfaces/heroUpdate';
...
export const delHero = async (ctx: Context, next: () => void) => {
    const payload: IHeroIDRequest = ctx.request.body;
...
export const heroDetails = async (ctx: Context, next: () => void) => {
    const payload: IHeroIDRequest = ctx.request.body;
    ctx.state.data = await service.getHeroDetails(payload);
    next();
};
